...
Итак к примеру у нас есть массив menu с конейнерами item:

```html

		<div data-menu="array" data-menu-class_menu="class">
...
Как видно из примера выше свойство `class_menu` является свойством массива (arrayProps - в описании приложения), а свойства class_item и text_item - свойствами контейнера item (props - в описании приложения).

В созданном экземпляре приложения вызов this.parent в свойстве контейнера указывает на контейнер, а свойство массива - на массив в котором оно расположено.
...
* для свойств массива: arrayProps:[ ["class_menu", "class", ""] ] - тотже тег что и у массива;

* для свойств контейнера: props:[ ["class_item", "class", ""], ["text_item", "text", "a:first-of-type"] ]

Теперь в html разметке у нас останется только массив и контейнеры:
