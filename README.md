...
Вызов метода getProp() вернет this.groupChild
...
**getProp()** - возвращает компонент который сейчас отображается,  а **removeProp()** - убирает его из видимости, если это контейнер в массиве (renderType == "container-inner") удаляет его;
