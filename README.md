...
* `remove(withChild)` - удаляет контейнер, если передать параметром whithChild=true удаляет также дочерние контейнеры, находящиеся в свойствах с типами: "group" и "render-variant", можно удалить только контейнер находящийся в массиве (renderType="contaiter-inner");
...
В нем каждый ключ ето один из возможных url для данного приложения. Знак : в начале слова говорит что эта чать url - является параметром и с ней сравнения не требуется, требуется только ее присутствие на данной позиции.
...
* `first` - имена компонентов, которые есть в html разметке на данном адресе сервера, чтобы взять из них шаблоны.
* `routComponen`- объект с названиями элементов в которых переключаются компоненты на данном url, например:
```javascript
		routComponent: {
			
			router_carts: "carts",  //найдет div элемент в котором есть data-router_carts="router" и на данном url вставит в него компонент carts
...
* `templatePath` - путь к файлу с шаблонами для компонентов, шаблонов которых нет на данном адресе url.
...
Перед инициализацией приложения роутер также как и переключении url смотрит каой сейчас адрес, и перестраивает приложение таким образом, чтобы поместить все компоненты, которых нет в свойстве first
в специальный объект fetchComponents и далее уже, и при инициализации приложение сначала создаст компоненты которых нет в объекте fetchComponents, а после ответа сервера на второй запрос по адресу templatePath создаст все остальные.
