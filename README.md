...
* `first` - имена компонентов, которые есть в html разметке на данном адресе сервера, чтобы взять из них шаблоны. Используется при инициализации приложения.
...
* `templatePath` - путь к файлу с шаблонами для компонентов, шаблонов которых нет на данном адресе url. Используется при инициализации приложения.
...
Перед инициализацией приложения, роутер также как и при переключении url смотрит какой сейчас адрес, и перестраивает приложение таким образом, чтобы поместить все компоненты, которых нет в свойстве first
