...
Htmlix - теперь Brother.js
**Brother.js** - яваскрипт фреймворк основанный на data- свойствах html документа. 
...
Brother.js это javascrit frontend framework для создания как различных элементов страницы, так и для одностраничных приложений. 
...
<!--
Примеры приложений на Brother.js:
...
Для создания приложения Brother.js необходимо создать экземпляр **new Brother.jsState( StateMap)** передав в него объект с описанием приложения  StateMap.
...
Компонентами в **Brother.js** выступают контейнеры и массивы. Контейнер это html элемент с различным набором свойств. 
...
window.onload = function(){
	...
	var HM = new Brother.jsState(StateMap); 
...
* `rootLink` - корневая ссылка на экземпляр приложения Brother.jsState;
...
# Методы массива Brother.jsArray 
...
window.onload = function(){
	...
	var HM = new Brother.jsState(State); 
...
* "group" - возвращает массив с объектами ключами которых будут названия свойств, а значения,
 данные полученные из этих свойств использует метод this.groupChild[i].getAllProps(),
 если передать в параметром mapObject={key1: "", key2: {ke1: ""} } `.getProp(mapObject)`,
...
что в свойстве должна быть ссылка на виртуальный массив данной группы this.groupArray если в данном свойстве ее нет, то он попытается отыскать ее в свойствах смежных контейнеров, если нен найдет то не сработает.
Поэтому при создании разметки, в свойстве с типом group нулевого контейнера, должен находиться контейнер который здесь будет отображаться, чтобы запомнить на него ссылку, 
...
* `.createInGroup(props, insertLocation)` - создает контейнер в виртуальном массиве и затем добавляет в группу на указаную позицию, если не указать добавит в конец, для работы требует наличие поля this.groupArray
...
Вызов метода setProp(val1, val2) - в свойстве с типом group:
* если передать уже созданный контейнер добавит его в группу на позицию val2, если не указать, добавит в конец вызвав метод `.addToGroup(container, insertLocation)`
* если передать обект с начальными данными для свойств в формате ключ - значение, вызовет метод `.createInGroup(props, insertLocation)`
* если передать массив с обектми arrayWithObjects вызовет метод `reuseGroup(arrayWithObjects)`

Вызов метода removeProp(val)
* если не передать параметр вызовет `.clearGroup()`
* если передать вызовет `.removeFromGroup(groupID)`
...
* `.render(nameComponent=string)` - отображает компонент с именем "nameComponent";
* `.renderByContainer(containerLink=container)` - отображает контейнер из виртуального массива по ссылке;
...
* objWidthProps - объект с новыми значениями для свойств, где имена ключей объекта должны совпадать с названиями устанавливаемых свойств, objWidthProps.componentName - обязательное поле объекта,
 которое содержит имя отображаемого компонента, 
 если отображаемый компонент является контейнером из виртуального массива здесь указывается имя виртуального массива, если отображаемый компонент является обычным массивом, 
 здесь также нужно указать поле data - массив с объектами, ключами которых являются новые свойства для контейнеров.
* если `objWidthProps.componentName` - обычный контейнер - устанавливает ему свойства совпавшие с ключами в объекте objWidthProps,
вызвав у него метод `setAllProps(objWidthProps)`, 
* `если objWidthProps.componentName` - виртуальный массив то удалит старый отображаемый контейнер и создаст новый из виртуального массива, а также установит его свойства,
из ключей объекта objWidthProps, вызвав метод массива `add(objWidthProps)`,
* если `objWidthProps.componentName` - обычный массив, отображает в свойстве данный массив и вызывает у него метод `reuseAll(objWidthProps.data)`,
передав в него массив `data` из объекта `objWidthProps` с новыми данными для контеййнеров.



**.setProp(newContainer)** - метод определит тип данных и затем вызовет нужный метод из перечисленных выше,
 * для текста .render (переключает одиночные контейнеры и массивы), 
* для контейнера из виртуального массива .renderByContainer, 
Также можно не только изменить отображаемый компонент, но сразу установить ему новые данные для свойств, передав объект: 
 * для одиночного контейнера и контейнера из виртуального виртуального массива {componentName: "название компонента", prop_name1: "новые данные", prop_name2: "новые данные",............},
 * для обычного массива   {componentName: "название массива", data: [{prop_name1: "новые данные", prop_name2: "новые данные",............}, {prop_name1: "новые данные", prop_name2: "новые данные",............}, ............]},

* **getProp()** - если this.renderChild.type == "container"  вызывает метод .getAllProps(), если this.renderChild.type == "array"  вызывает метод .getAll(),
тем самым получа значения всех свойств из отображаемого элемента, если передать в параметром mapObject={key1: "", key2: {ke1: ""} } `.getProp(mapObject)`,
 вернет свойства только совпадающие по названиям с key, таким образом можно получить развернутый объект со всеми значениями свойств либо со всеми нужными значениями свойств из отображаемого элемента; 
...
В Brother.js можно использовать роутер для обновления истории, а также смены отображаемых компонентов в зависимости от переданного url.

Роутер создается в экжемпляре приложения в свойстве `.router`, для этого нужно использовать  функцию `Brother.jsRouter(StateMap, routes)` которя возвращает 
экземпляр `Brother.jsState` с новым свойством `.router`. Первый параметр StateMap - объект описания приложения, routes - объект с картой роутов, ключами которого 
...
Выше приведен фрагмент кода из <a href="https://github.com/SergeyOvechkin/Brother.js/wiki/2.1-%D0%A3%D1%80%D0%BE%D0%BA-%E2%84%962.1-%D0%A1%D0%BE%D0%B7%D0%B4%D0%B0%D0%BD%D0%B8%D0%B5-%D0%BF%D1%80%D0%BE%D1%82%D0%BE%D1%82%D0%B8%D0%BF%D0%B0-%D0%BA%D0%BB%D0%B8%D0%B5%D0%BD%D1%82%D1%81%D0%BA%D0%BE%D0%B9-%D1%87%D0%B0%D1%81%D1%82%D0%B8-%D0%B8%D0%BD%D1%82%D0%B5%D1%80%D0%BD%D0%B5%D1%82-%D0%BC%D0%B0%D0%B3%D0%B0%D0%B7%D0%B8%D0%BD%D0%B0-%D0%BD%D0%B0-Brother.js">Пошагового руководства по созданию клиентской части SPA магазина</a>
...
**Brother.js** - находится в стадии тестирования, поэтому не исключена возможность возникновения непредвиденных ошибок, однако несмотря на это, его уже сейчас можно использовать в различных проектах.
