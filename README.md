...
Методы **setProp("newProp")** и **removeProp()** работают аналогично:

* `setProp("newProp")` - для "text","html","inputvalue", "select","checkbox", "radio", "data", 'href', 'src', "id", "style" - утанавливают переданное в них значение свойству;
...
* `removeProp()` - "text","html","inputvalue", "select", "data", 'href', 'src', "id", "style" - устанавливают данные свойства - "", можно также использовать метод setProp("");
* для "checkbox", "radio", устанавливают false;
...
либо использовать метод `.createNewGroup(groupArr, componentName)`
...
если необходимо создать пустое свойство для всех контейнеров и только в процессе добавлять туда контейнеры, необходимо создать шаблон с типом "template" и (style="display: non") для того чтобы запомнить ссылку на массив группы. 
* `.createNewGroup(groupArr, componentName)` - если виртуальный массив данной группы this.groupArray совпадает с именем `componentName` то вызывает метод `reuseGroup(groupArr)` , если не совпадает, удаляет все из группы и создает 
...
* если передать arrayWithObjects - массив с обектми, ключами которых являются имена свойств - вызовет метод `reuseGroup(arrayWithObjects)`
* если передать объект с полями `val.componentName`-имя виртуального массива, `val.group`- массив с обьектами, ключами которых являются имена свойств - вызовет метод `.createNewGroup(val.group, val.componentName)`
...
* `если objWidthProps.componentName` - виртуальный массив то удалит старый отображаемый контейнер в свойстве и создаст новый из виртуального массива, а также установит его свойства,
...
но что если мы не хотим писать все шаблоны на одной станице, чтобы не создавать путаницы, для этого можно "догрузить" остальные шаблоны - неиспользуемые при первой загрузке данных с сервера, как это зделать?
...
Удалили из основной разметки компонент variant2 а также свойство со стилями display: none, т.к. оно больше не пригодится
Далее создадим файл template.html и добавим в него шаблон для компонента  variant2-style
...
И после клика по кнопке мы меняем отображаемый компонент в свойстве, также можно не только поменять отображаемый компонент, но и сразу установить ему какие либо свойства:
